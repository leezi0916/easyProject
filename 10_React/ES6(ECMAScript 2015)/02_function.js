...
//js에서는 함수도 하나의 값(변수)으로 취급된다. 함수인지 변수인지 잘 판단해야함!!
let _hello = hello;
console.log(_hello);
_hello();
...
// 함수를 변수에 넣으면 변수에따라 호이스팅이 안됨 (const)
...
//왜? 그러면 undefined를 따로 사용 하지 않고 앞에 았는 매개변수만 사용하면 되기 때문!
function greet2(name, hobby = "독서"){
    console.log(`${name}님의 취미는 ${hobby}군요.`); 
}

greet2("최민수");
//앞쪽 배치시 greet2(undefined, "최민수"); 이런식 으로 됨 ㅜㅜ
